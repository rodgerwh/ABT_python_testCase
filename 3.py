...
async def main():
    ...
    # Определяем новую функцию для обработки запросов, чтобы не модифицировать handle_request в Server
    async def handle_request(value):
        async with semaphore:  # Используем семафор для последовательной обработки запросов
            await server.handle_request(value)

    # имитируем запуск 10 запросов к серверу
    requests = [handle_request(value) for value in range(10)]
